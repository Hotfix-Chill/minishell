...
/*   Updated: 2025/12/02 01:24:31 by pjelinek         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool is_builtin(const char *cmd)
{
    ...
    if (ft_memcmp(cmd, "cd", 3) == 0)
        return true;
    if (ft_memcmp(cmd, "pwd", 4) == 0)
        return true;
    if (ft_memcmp(cmd, "export", 7) == 0)
        return true;
    if (ft_memcmp(cmd, "unset", 6) == 0)
        return true;
    if (ft_memcmp(cmd, "env", 4) == 0)
        return true;
    if (ft_memcmp(cmd, "exit", 5) == 0)
        return true;
...
/* ======================================================== */
/*   Build commands incl. argv, redirs, builtin flag         */
/* ======================================================== */
void    *debug_build_commands(t_data *data)
{
    ...
    while (i < count)
    {
        ...
        /* >>> builtin detection <<< */
        node->builtin = is_builtin(node->argv[0]);
