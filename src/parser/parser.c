...
/*   Updated: 2026/01/16 14:48:11 by abita            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (tok)
		// loops through all tokens and processes them based on their type
	{
		...
		else if (tok->typ == TOKEN_REDIR) // REDIR_TOKEN
		{
			if (!tok->next || tok->next->typ != TOKEN_WORD)
				return (EXIT_FAILURE);
			if (add_redir_to_cmd(*curr_cmd, tok, tok->next,
					data) != EXIT_SUCCESS)
				return (EXIT_FAILURE);
			tok = tok->next->next;
		}
		...
		{
			if (add_arg_to_cmd(*curr_cmd, tok->content, tok->quote) != EXIT_SUCCESS)
				return (EXIT_FAILURE);
			tok = tok->next;
		}
		else
